...
list :: CommandList
list =	[ ("clan-add"		, (clanAdd	, 4	, Peon	, "\"tag\" \"name\" \"irc channel\" \"homepage url\""
...
clanAdd _ mess Info{echo} ComState{conn} = case mread mess :: Maybe InsertClan of
	...
	_	-> echo $ view "clan-add" "Error in syntax."

clanUpdate _ mess Info{echo} ComState{conn} = case mread mess :: Maybe InsertClan of
	Just (InsertClan tag name irc homepage) | all (not . null) [tag, name]	-> do
		x <- run conn "UPDATE clans SET tag = ?, name = ?, irc = ?, homepage = ? WHERE LOWER(tag) = LOWER(?)"
			[toSql tag, toSql name, toSql irc, toSql homepage, toSql tag]
		commit conn
		echo $ name ++ ": " ++ if x > 0 then "modified." else "not found."

	_	-> echo $ view "clan-update" "Error in syntax."

clanDel _ clan Info{echo} ComState{conn} = let
